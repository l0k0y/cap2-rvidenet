...
for i in range(0,len(dataset_split_names)):
    ...
    dataset_path = '/database/iyj0121/dataset/ELRVD/MOT/MOT17-{}/img1/'.format(dataset_split_names[i])
    save_path = '/database/iyj0121/dataset/ELRVD/MOT/MOT17-{}_raw/'.format(dataset_split_names[i])
    ...
    for j in range(len(srgb_imgs_list)):
        ...
        #save_result.save(save_path + base_name[:-4] + '.raw')
        raw_file_path = save_path + base_name[:-4] + '.raw'
        raw_bayer.astype(np.uint16).tofile(raw_file_path)
