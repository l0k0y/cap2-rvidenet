...
parser.add_argument('--output_dir', type=str, default="/database/iyj0121/result/tcsvt/CRVD_baseline_result_texture_loss_final_final/", help='output path')
...
model = torch.load("/database/iyj0121/ckpt/rvidnet/finetuning_CRVD_rggb_baseline_texture_loss/model_epoch300.pth").cuda()

iso_list = [1600,3200,6400,12800,25600]

total_avg_raw_psnr = 0
total_avg_raw_ssim = 0
...
for iso in iso_list:
    ...
    for scene_id in range(7,11+1):
        ...
        for i in range(1,7+1):
            ...
            for j in range(-1,2):
                if (i+j)<1:
                    #raw = cv2.imread('/database/iyj0121/dataset/CRVD_data/scene{}/ISO{}/frame1_noisy0.raw'.format(scene_id, iso),-1)
                    raw_path = '/database/iyj0121/dataset/CRVD_data/scene{}/ISO{}/frame1_noisy0.raw'.format(scene_id, iso)
                    raw = np.fromfile(raw_path, dtype=np.uint16).reshape(1080, 1920)
                    input_full = np.expand_dims(pack_rggb_raw(raw), axis=0)
                    frame_list.append(input_full)
                elif (i+j)>7:
                    #raw = cv2.imread('/database/iyj0121/dataset/CRVD_data/scene{}/ISO{}/frame7_noisy0.raw'.format(scene_id, iso),-1)
                    raw_path = '/database/iyj0121/dataset/CRVD_data/scene{}/ISO{}/frame7_noisy0.raw'.format(scene_id, iso)
                    raw = np.fromfile(raw_path, dtype=np.uint16).reshape(1080, 1920)
                    input_full = np.expand_dims(pack_rggb_raw(raw), axis=0)
                    frame_list.append(input_full)
                else:
                    #raw = cv2.imread('/database/iyj0121/dataset/CRVD_data/scene{}/ISO{}/frame{}_noisy0.raw'.format(scene_id, iso, i+j),-1)
                    raw_path = '/database/iyj0121/dataset/CRVD_data/scene{}/ISO{}/frame{}_noisy0.raw'.format(scene_id, iso, i+j)
                    raw = np.fromfile(raw_path, dtype=np.uint16).reshape(1080, 1920)
                    input_full = np.expand_dims(pack_rggb_raw(raw), axis=0)
                    frame_list.append(input_full)
            ...
            test_result = depack_rggb_raw_crvd(test_result)

            #test_gt = cv2.imread('/database/iyj0121/dataset/CRVD_GT/scene{}/ISO{}/frame{}_clean_and_slightly_denoised.tiff'.format(scene_id, iso, i),-1).astype(np.float32)
            gt_path = '/database/iyj0121/dataset/CRVD_GT/scene{}/ISO{}/frame{}_clean_and_slightly_denoised.raw'.format(scene_id, iso, i)
            test_gt = np.fromfile(gt_path, dtype=np.uint16).reshape(1080, 1920)
            ...
            output = Image.fromarray(np.uint16(output))
            #save_result.save(args.output_dir+'ISO{}/scene{}_frame{}_denoised_raw.raw'.format(iso, scene_id, i))
            save_path = args.output_dir+'ISO{}/scene{}_frame{}_denoised_raw.raw'.format(iso, scene_id, i)
            ...
            # denoised_raw_frame = preprocess(np.expand_dims(pack_rggb_raw(output),axis=0))
            ...
            # gt_raw_frame = np.expand_dims(pack_rggb_raw(test_gt*(2**12-1-240)+240), axis=0)
    ...
    total_avg_raw_psnr += scene_avg_raw_psnr
    total_avg_raw_ssim += scene_avg_raw_ssim
...
total_avg_raw_psnr = total_avg_raw_psnr/5
total_avg_raw_ssim = total_avg_raw_ssim/5

print('total average raw psnr:{},total average raw ssim:{}'.format(total_avg_raw_psnr,total_avg_raw_ssim))
context = 'total average raw psnr:{},total average raw ssim:{}'.format(total_avg_raw_psnr,total_avg_raw_ssim) + '\n'
