...
parser.add_argument('--input_dir', type=str, default="/database/iyj0121/dataset/hanhwa/raw_0317/", help='input path') #     "/database/iyj0121/dataset/hanhwa/raw_0317/"
parser.add_argument('--output_dir', type=str, default="/database/iyj0121/dataset/4k_vnt/DPC_Dark_Moving_Chart_Dynamic_finetune_noise_estimation_crvd_method_neif_on_4k_calibration_gain_6_30_hcg_scene_from_1_to_6_video_model_input_full/", help='output path')
...
#isp = nn.DataParallel(isp, device_ids=[0, 1]).cuda()
...
for frame_idx, input_file in enumerate(input_files):
    ...
    for j in range(-1, 2):
        ...
        input_full = pack_rggb_raw_(raw)
        #frame_list.append(input_full)
        frame_list.append(np.expand_dims(input_full, axis=0))

    input_data = np.stack(frame_list, axis=1) 
    print("input_data shape:", input_data.shape)
    #test_result = test_big_size_raw(input_data, model, patch_h=256, patch_w=256, patch_h_overlap=64, patch_w_overlap=64)
    input_tensor = torch.from_numpy(input_data).permute(0, 1, 4, 2, 3).float().cuda()
    ...
    print(f"Inference time for frame {frame_idx}: {processing_time:.4f} seconds")
    output_tensor = output_tensor.squeeze().cpu().numpy()
    test_result = depack_rggb_raw(output_tensor)
    save_result = (test_result * ((2**12-1)- 300) + 300).astype(np.uint16)
