...
parser.add_argument('--input_dir', type=str, default="/database/iyj0121/dataset/4k_vnt/DPC_Dark_Moving_Chart_Dynamic/", help='input path') #     "/database/iyj0121/dataset/hanhwa/raw_0317/"
parser.add_argument('--output_dir', type=str, default="/database/iyj0121/dataset/4k_vnt/DPC_finetune_noise_estimation_crvd_method_neif_on_4k_calibration_gain_6_30_hcg_scene_from_1_to_6_video_npy/", help='output path')
...
for frame_idx, input_file in enumerate(input_files):
    ...
    test_result = test_result.astype(np.float32)
    print(test_result.min(), test_result.max(), test_result.dtype)

    save_pass = os.path.join(args.output_dir, 'denoised_raw_frame_{:06d}.npy'.format(frame_idx))
    np.save(save_pass, test_result)
    print("Saved denoised output to:", save_pass)
